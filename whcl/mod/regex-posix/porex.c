...
/*
  This file implements a whcl binding for regcomp(3) and regexec(3).
*/
#include <assert.h>
...
static void rx_posix_finalizer( cwal_engine * e, void * m ){
  /* MARKER(("freeing regex_t@%p sizeof(regex_t)=%d\n", m, (int)sizeof(regex_t))); */
  regfree((regex_t *)m);
  cwal_free2(e, m, (cwal_size_t)sizeof(regex_t));
}
...
*/
static int rx_cb_compile( cwal_callback_args const * args,
                            cwal_value **rv ){
  char const * pat = NULL;
  regex_t re;
  regex_t * newRe;
  ...
  cwal_value * natv;
  cwal_value * proto = cwal_args_state(args, &rx_prototype_id);
  assert(proto);
  if(!proto){
  ...
  rc = rx_flag_arg(args->engine, RX_FLAG_ARG_COMPILE,
                     args->argc>1 ? args->argv[1] : 0, &flags,
                     &zExecFlags[0]);
  if(rc) return rc;
  rc = regcomp( &re, pat, flags);
  if(rc){
    return rx_posix_cb_error(args, "regcomp()", rc, &re);
  }
  ...
  if(!natv){
    regfree(&re);
    rx_posix_finalizer(args->engine, newRe);
    rc = CWAL_RC_OOM;
  }else{
    cwal_value_ref(natv);
...
static int whcl_module_init_regex_posix( whcl_engine * const el, cwal_value ** module ){
  return rx_module_init(el, module);
}

WHCL_MODULE_REGISTER3(regex_posix, regex-posix, whcl_module_init_regex_posix);
