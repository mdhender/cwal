...
static int whcl_module_init__template( whcl_engine * const el, cwal_value ** rv ){
  int rc = 0;
  cwal_engine * const ec = whcl_engine_cwal(el);
  cwal_value * mod = cwal_new_object_value(ec);
  ...
  /* Install some funcs... */
  {
    whcl_func_def const funcs[] = {
      /* Install functions... */
      WHCL_FUNC2("one", cb_one),
      WHCL_FUNC2("two", cb_two),
...
WHCL_MODULE_REGISTER_(_template);
