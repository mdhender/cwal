...
/**
   Example loadable module using the cwal_convert.hpp templates
...
/** @internal
   Convenience macros.
*/
#define WHCL_UNUSED_VAR __attribute__((__unused__)) /* avoiding unused var in non-debug build */

#define CERR std::cerr << __FILE__ << ':' << std::dec << __LINE__ << ':' << __FUNCTION__ << "(): "
#define COUT std::cout << __FILE__ << ':' << std::dec << __LINE__ << ':' << __FUNCTION__ << "(): "
#include "wh/cwal/cwal_convert.hpp" /* must come after libwhcl.h */
...
/**
   Custom client-side native type to bind to whcl. We use
...
/**
   ...
   module interface, is whcl_module_init_{MODULE_NAME}.
*/
static int whcl_module_init_sample_cpp( whcl_engine * const el, cwal_value ** rv ){
  ...
  cwal_engine * const ec = whcl_engine_cwal(el);
  ...
  try {
...
#else
    typedef cwal::CbToCb< whcl_cb_sleep > fSleep;
#endif
    typedef cwal::CbToCb< whcl_cb_mssleep > fSleepMs;
    /*
    ...
    */
    ...
    /**
       ...
       overloaded variants for use with whcl's "new" builtin
       command. This is not directly part of the ProtoMaker's API
       because plugging in the ctor is whcl-level functionality,
       whereas ProtoMaker is "pure cwal". That said, when ProtoMaker
       sees that whcl was included, it adds ProtoMaker::whclCtor() to
       support us...
    */
    typedef cwal::CbRule<
    ...
    pm
      ...
      .toCb<mFuncStr>("func-str")
      .toCb<fSleep>("sleep")
      .toCb<fSleepMs>("mssleep")
      .toCb<mSelf>("self")
      .toCb<mPassPtr>("pass-ptr")
      .toCb<mPassRef>("pass-ref")
      .toCb<quasiMember>("non-member")
      .toCb<quasiMember2>("non-member2")
      .toCb<mGetter>("get-value")
      .toCb<mSetter>("set-value")
      .toCb<mGetterConst>("get-const-value")
    ...
    */
    ...
    rc = whcl_ctor_callback_set( se, myProto, Ctors::callback );
    if(rc) goto end;
#endif
    ...
    /*
      In whcl (but not vanilla cwal), myProto will automatically inherit
      whcl's built-in Object class, but if we want to re-assign or
...
#define SET(KEY) \
    ...
    if(rc) goto end
#define FUNC(NAME,FP)                                   \
    v = cwal::newFunctionValue< FP >( ec );          \
    SET(NAME)
    
    
    /* Install some arbitrary value... */
    v = cwal_new_integer(ec, 42);
    SET("theAnswer");
    ...
    */
    if(0){
      ...
      assert(ctor && "We set this above via pm.whclCtor()");
      rc = whcl_stash_hidden_member( ctor, myProto )
        /* This will keep the prototype alive so long as nobody
           calls cwal_clear_props() on ctor. */;
    }

    if(0){
      /* Just testing... if the following compiles at all, the chances are
         very high that it works. This block is only disabled by default
         because its output interferes with automated testing. */
      WHCL_UNUSED_VAR MyType * my = 0;
      cwal_value * myV = 0;
      rc = whcl_ctor_apply(el, myProto, 0, 0, &myV)
        /* Runs myProto's designated ctor, as if we had called "new"
           from script code, passing it no arguments, and gives us the result
           Value via &myV. */;
      if(rc){
        assert(!myV);
        goto end;
      ...
      cwal_ref(myV);
      my = cwal::toNative<MyType>(ec, myV);
      ...
      */
      assert( cwal::toCwal<MyType>(ec, my) == myV );
      assert( cwal::toCwal<MyType*>(ec, my) == myV );
      assert( cwal::toCwal<MyType const *>(ec, my) == myV );
      assert( cwal::toCwal<MyType &>(ec, *my) == myV );
      assert( cwal::toCwal<MyType const &>(ec, *my) == myV );
      ...
      */
      assert( cwal::toNative<MyType>(ec, myV) == my );
      assert( cwal::toNative<MyType *>(ec, myV) == my );
      assert( cwal::toNative<MyType const *>(ec, myV) == my );
      assert( cwal::toNative<MyType &>(ec, myV) == *my );
      assert( cwal::toNative<MyType const &>(ec, myV) == *my );
      ...
      if(0){ // finalization, approach #1:
        ...
        */
        ...
        assert(!cwal::toNative<MyType>(ec, myV) );
...
#undef FUNC
    ...
    rc = cwal_exception_setf(ec, CWAL_RC_EXCEPTION,
...
/**
   ...
   whcl_module_load() and friends...
*/
WHCL_MODULE_REGISTER_(sample_cpp);
