/*!
  ...
  Site-specific customizations are added at the end.
*/
...
;hljs.registerLanguage("plaintext",t)})();/*! `tcl` grammar compiled for Highlight.js 11.4.0 */
...
;hljs.registerLanguage("tcl",e)})();/*! `bash` grammar compiled for Highlight.js 11.4.0 */
...
})();hljs.registerLanguage("json",e)})();/*! `css` grammar compiled for Highlight.js 11.4.0 */
(()=>{var e=(()=>{"use strict"
...
;hljs.registerLanguage("makefile",e)})();/*! `diff` grammar compiled for Highlight.js 11.4.0 */
...
/* added by stephan@wanderinghorse.net for his own site... */
/**
...
This file:
<script src="$baseurl/raw?ci=trunk&filename=doc/highlightjs/highlight-cwal.min.js"></script>
...
if( /\/doc\/\w+/.test(window.location.pathname)
  ...
  document.addEventListener(
    ...
    function(event){
      /**
        Jump through some hoops to squelch hljs's console warnings
        about unknown language blocks which we style ourselves.  This
        approach also stops hljs from brute-force guessing what any
        given block is, which is an unnecessary CPU hog/battery drain.
      */
        let whclLang = hljs.getLanguage('tcl').rawDefinition(hljs);
        //console.debug('tcl =',whclLang);
        try {whclLang = JSON.parse(JSON.stringify(whclLang))}catch(e){}
        whclLang.keywords = {
            built_in: [
            // Built-in commands:
            'affirm', 'array', 'assert', 'break',
            'catch', 'concat', 'continue', '__debug',
            'decl', 'decr', 'do', 'echo',
            'exception', 'exit', 'eval', 'expr',
            'for', 'foreach', 'if', 'incr',
            'info', 'new', 'object', 'pragma',
            'proc', 'return', 'set', 'throw',
            'unset', 'while'
            ],
            literal: [
                // Built-in/magic values:
                'argv', 'true', 'false', 'null', 'this', 'undefined',
                'using', 'whcl', '__COLUMN', '__LINE', '__FILE',
                '__FILEDIR', '__FLC', '__prototype'
            ]
        };
        whclLang.contains.push(hljs.C_BLOCK_COMMENT_MODE);
        whclLang.contains.push(hljs.APOS_STRING_MODE);
        whclLang.name = "whcl";
        whclLang.aliases = [];
        //console.debug('wchlLang =',whclLang);
        hljs.registerLanguage('whcl', ()=>whclLang);
        [
            "bash", "c", "console", "cpp",
